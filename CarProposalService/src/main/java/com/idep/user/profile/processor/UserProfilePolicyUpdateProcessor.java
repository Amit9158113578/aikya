 ...
 public class UserProfilePolicyUpdateProcessor implements Processor {
   ...
   public void process(Exchange exchange) {
     try {
       String userPolicyDetails = (String)exchange.getIn().getBody(String.class);
       JsonNode userPolicyDetailsNode = this.objectMapper.readTree(userPolicyDetails);
       updatePolicyDocument(userPolicyDetailsNode);
     } catch (Exception e) {
       this.log.error("Exception at UserProfilePolicyUpdateProcessor : ", e);
     } 
   }
   
   public void updatePolicyDocument(JsonNode policyResNodeDetails) {
     try {
       JsonNode userProfileDetailsNode = this.profileServices.getUserProfileByUkey(policyResNodeDetails.get("uKey").asText());
       if (userProfileDetailsNode != null) {
         JsonNode userPersonalInfo = userProfileDetailsNode.get("userProfile");
         ObjectNode policyDetailsNode = this.objectMapper.createObjectNode();
         policyDetailsNode.put("filePath", policyResNodeDetails.get("filePath").asText());
         policyDetailsNode.put("contentRepoType", policyResNodeDetails.get("contentRepoType").asText());
         ...
         boolean status = this.profileServices.updatePolicyRecordByPkey(mobile, policyResNodeDetails.get("pKey").asText(), policyDetailsNode);
         this.log.info("policy details updated status : " + status);
       } else {
         this.log.error("User profile not found : UserKey" + policyResNodeDetails.get("uKey").asText() + "Pkey " + policyResNodeDetails.get("pKey").asText());
       } 
     } catch (Exception e) {
       this.log.error("Failed to update policy document details", e);
     } 
   }
 }
