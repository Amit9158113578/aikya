...
public class PolicyTransHandler implements Processor {

	Logger log = Logger.getLogger(PolicyTransHandler.class.getName());
	...
	public void process(Exchange exchange) throws Exception {
		...
		try {
				...
				log.info("policy trans sync request : "+request);
				log.info("Request Headers : "+exchange.getIn().getHeaders());
				/**
				 * form a URL based on configuration and received headers
				 */
				//"http://192.168.0.10:8095/policytransaction";
				//String url = SyncGatewayURLLoader.syncGateTransAdminURL;
				String url= SyncGatewayURLLoader.syncGateTransPublicURL;
				...
				exchange.getIn().setHeader("transurl", url);
