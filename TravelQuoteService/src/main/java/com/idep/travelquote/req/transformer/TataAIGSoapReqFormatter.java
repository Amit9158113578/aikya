...
public class TataAIGSoapReqFormatter implements Processor {
	...
	public void process(Exchange exchange) throws Exception {
		...
		try {
			 ...
			 */
			log.info("Tata AIG ConfigData ::::::::::::::"+configData);
			...
			if (configData.has("carrierSOAPConfig"))
			{
				if(configData.get("carrierSOAPConfig").get("reqConfig").has("methodName"))
				{
					...
					log.info("Tata AIG methodName::::::::::::-----"+methodName);
				}
				...
				if(configData.get("carrierSOAPConfig").get("reqConfig").has("methodParam"))
				{
					...
					log.info("Tata AIG methodParam::::::"+methodParam);
				}
				...
				if(configData.has("carrierSOAPConfig"))
				{
					...
					log.info("Tata AIG preparesoaprequest method gives response:::::::::---"+response);
					...
					log.debug("Final Tata AIG response: "+response ); 		  
				}
			}
		...
		{
			log.error(exchange.getProperty(TravelQuoteConstants.LOG_REQ).toString()+TravelQuoteConstants.TATAAIGSOAPREQFORM+"|ERROR|"+" Exception at SOAPRequestFormatter for request :"+response,e);
