...
public class CignaSOAPRequestFormatter implements Processor {
	
	  Logger log = Logger.getLogger(CignaSOAPRequestFormatter.class.getName());
	...
	public void process(Exchange exchange) throws ExecutionTerminator {
		...
		try {
		  ...
		  JsonNode configData = exchange.getProperty("cignaSoapReqConfDocument",JsonNode.class);
		  /**
		...
		{
			log.error(exchange.getProperty(ProposalConstants.LOG_REQ).toString()+ProposalConstants.SERVICEINVOKE+"|ERROR|CignaSOAPRequestFormatter|",e);
