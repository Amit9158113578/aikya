 ...
 public class PolicyDocumentReqProcessor implements Processor {
   ...
   public void process(Exchange exchange) {
     try {
       ...
       ((ObjectNode)reqNode).put("requestType", "CarPolicyDocumentRequest");
       ((ObjectNode)reqNode).putAll((ObjectNode)proposalDocNode);
       exchange.setProperty("logReq", "Car|" + proposalDocNode.findValue("carrierId").asText() + "|" + "POLICY SIGN" + "|" + proposalDocNode.findValue("policyNo").asText() + "|");
       exchange.setProperty("carPolicyDocumentRequest", reqNode);
       exchange.setProperty("userPolicyProfileData", reqNode);
       exchange.getIn().setBody(reqNode);
     } catch (Exception e) {
       this.log.error(String.valueOf(String.valueOf(String.valueOf(exchange.getProperty("logReq").toString()))) + "POLICYDOCREQPRO" + "|ERROR|" + "policy doc req processing failed :", e);
     } 
   }
 }
