...
import com.idep.policy.document.req.processor.PolicyDocUploaderProcessor;
...
public class PolicyDocumentResProcessor implements Processor
{
  ...
  public void process(Exchange exchange) throws ExecutionTerminator {
	    ...
	    try
	    {
	       ...
	       */
	      JsonNode policyDocReqNode = objectMapper.readTree(exchange.getProperty(ProposalConstants.HLTHPOLICYDOC_INPUT_REQ).toString());
	      
	      /**
	       * read carrier configuration from property
	       */
	      JsonNode policyConfigNode = objectMapper.readTree(exchange.getProperty(ProposalConstants.HLTHPOLICYDOC_CONFIG).toString());
	      ...
	      */
	      ...
	      if(filePath.length()>0)
	      {
		      ...
		      try
		      {
		    	  JsonNode proposalDoc= objectMapper.readTree(exchange.getProperty(ProposalConstants.HLTHPOLICYDOC_INPUT_REQ).toString());
		      ...
		      {
		    	 log.error(exchange.getProperty(ProposalConstants.LOG_REQ).toString()+ProposalConstants.POLICYDOCRES+"|ERROR|failed to uploading policy document at Alfresco|",e);
	    ...
	    {
	    	log.error(exchange.getProperty(ProposalConstants.LOG_REQ).toString()+ProposalConstants.POLICYDOCRES+"|ERROR|PolicyDocumentResProcessor|",e);
			throw new ExecutionTerminator();
	    }
  }
