...
public class AddNewMenuConfig implements Processor {
	ObjectMapper objectMapper = new ObjectMapper();
	Logger log = Logger.getLogger(AddNewMenuConfig.class.getName());
	...
	public void process(Exchange exchange) throws Exception {
		try{
			...
			log.info("RequestNode for add new Screen Config  : "+requestNode);
				String docId = "PospMenuConfg";
			JsonDocument configDoc = PospData.getDocBYId(docId);
			if(configDoc!=null){
				JsonObject docObj = JsonObject.fromJson(request);
			JsonArray screenConfigArray = configDoc.content().getArray(POSPServiceConstant.POSPMENUCONFIG);
			screenConfigArray.add(docObj.get(POSPServiceConstant.POSPMENUCONFIG));
			configDoc.content().put(POSPServiceConstant.POSPMENUCONFIG, screenConfigArray);
