...
/**
 * @author pravin.jakhi
 * this processor give disease as per plan-wise or without plan-wise
 *
 */
public class HealthDiseaseReqProcessor implements Processor {
	...
	public void process(Exchange exchange) throws Exception 
	{
		...
		{			
			...
			if(inputReqNode.has("carrierId") && inputReqNode.has("planId"))
			{
				try
				{
					DiseasePlanConfig = objectMapper.readTree(serviceConfig.getDocBYId("HealthPlanDiseaseConfig").content().toString());
					if(DiseasePlanConfig!=null)
					{
						if(DiseasePlanConfig.has(inputReqNode.get("carrierId").asText()+"-"+inputReqNode.get("planId").asText()))
						{
							diseaseQuery="select ProductData.* from ProductData where documentType='DiseaseMapping' and carrierId="+inputReqNode.get("carrierId").asText()+" "
									+ "and planId="+inputReqNode.get("planId").asText()+" ORDER BY preference";
						}
						else
						{
							diseaseQuery="select ProductData.* from ProductData where documentType='DiseaseMapping' and carrierId="+inputReqNode.get("carrierId").asText()+" ORDER BY preference";
						}
						try
						{
					...
					{
						log.error("HealthPlanDiseaseConfig document not found in DB");
					}
				}
				catch(Exception e)
				{
					log.error("failed to fetch HealthPlanDiseaseConfig document from DB ",e);
				}
		...
		{
			log.error("Error AT HealthDiseaseReqProcessor : ",e);
		}	
