...
import com.idep.lifequote.req.transformer.SOAPResponseFormatter;
...
public class SOAPResponseFormatter  implements Processor {
	...
	public void process(Exchange exchange) throws Exception {
		...
		try
		{
			//log.info("Inside SOAPResponseFormatter 1");
			...
			/*JsonNode resConfigData = exchange.getProperty(LifeQuoteConstants.CARRIER_QUOTE_REQ_MAP_CONF,JsonNode.class);*/
			String productId=exchange.getProperty("productId").toString();
			JsonNode resConfigData=objectMapper.readTree(serverConfig.getDocBYId("LifeQuoteRequest-9-"+productId).content().toString());
			//log.info("JSON NODE : "+resConfigData); 
			if (resConfigData.has("carrierSOAPConfig"))
			  {
				 //log.info("Inside carrierSOAPConfig 1 and ResConfig");
				  if(resConfigData.get("carrierSOAPConfig").get("resConfig").has("startTagName"))
				  {
					  //log.info("Inside carrierSOAPConfig 1 and ResConfig. checking startTagName");
