...
public class HDFCPolicySOAPRequestFormatter implements Processor {
	Logger log = Logger.getLogger(HDFCPolicySOAPRequestFormatter.class.getName());
	...
	public void process(Exchange exchange) {
		try {
			String reqNode = (String)exchange.getIn().getBody();
				...
				if(!inputRequest.has("documentType")){
					requestType = "HealthPolicyRequest";
				}
				else{
					requestType = inputRequest.findValue("documentType").asText();
				}
				...
				if (configurations != null) {
					...
					requestFormed = requestFormed.replace("<PartnerCode", "<PartnerCode xmlns=\"http://schemas.datacontract.org/2004/07/ServiceObjects\"")
							...
							.replace("<PaymentId", "<PaymentId xmlns=\"http://schemas.datacontract.org/2004/07/TransactionVerificationLibrary\"")
							.replace("<TransactionId", "<TransactionId xmlns=\"http://schemas.datacontract.org/2004/07/TransactionVerificationLibrar\"");
