 ...
 public class CarPolicyDocDownloadProcessor
   implements Processor
 {
   Logger log = Logger.getLogger(CarPolicyDocDownloadProcessor.class.getName());
     ...
     try {
       ...
       if (requestNode.has("responseCode") && requestNode.getKey("responseCode").equals(DocumentDataConfig.getConfigDocList().get("ResponseMessages").get("ResponseCodeSuccess").asText())) {
         ...
         if (requestNode.has("downloadPolicy") && requestNode.getKey("downloadPolicy").equalsIgnoreCase("Y")) {
           JsonDocument policyDocDownConfig = this.serverConfig.getDocBYId("CarPolicyDocDownloadConfig-" + requestNode.findValueAsText("carrierId") + "-" + requestNode.findValueAsText("productId"));
           if (policyDocDownConfig != null) {
             ...
             if (extendedconfigDocNode.has("transformRequest")) {
               JsonNode indexArrayNode = null;
               JsonNode docsArrayNode = null;
               String policyIndexValue = null;
               JsonNode transformedNode = requestNode.findValue(extendedconfigDocNode.get("transformRequest").asText("requestNode"));
               if (extendedconfigDocNode.get("transformRequest").has("subNode1"))
                 indexArrayNode = transformedNode.findValue(extendedconfigDocNode.get("transformRequest").asText("subNode1")); 
               if (extendedconfigDocNode.get("transformRequest").has("subNode2"))
                 docsArrayNode = transformedNode.findValue(extendedconfigDocNode.get("transformRequest").asText("subNode2")); 
               for (int i = 0; i <= docsArrayNode.size(); i++) {
                 if (docsArrayNode.get(i).get("name").asText().substring(0, 20).equals(extendedconfigDocNode.get("transformRequest").asText("validateFields"))) {
             ...
             if (extendedconfigDocNode.has("concatenate") && 
               extendedconfigDocNode.get("concatenate").has("lastIndexOf"))
               if (requestNode.has(extendedconfigDocNode.get("concatenate").get("lastIndexOf").asText("downloadLink"))) {
                 ...
                 downloadURL = String.valueOf(String.valueOf(String.valueOf(downloadURL))) + pdfName;
             ...
             ExtendedJsonNode failure = (new ExceptionResponse()).configDocMissing("config document not found for carrier :CarPolicyDocDownloadConfig-" + requestNode.findValueAsText("carrierId") + "-" + requestNode.findValueAsText("productId"));
   ...
   public void updatePolicyDocument(ExtendedJsonNode extendedJsonNode, String downloadURL) throws Exception {
     try {
       ...
       if (userProfileDetailsNode != null) {
         ...
         this.log.error("User profile not found : UserKey" + extendedJsonNode.findValueAsText("uKey") + "Pkey " + extendedJsonNode.findValueAsText("pKey"));
       } 
     } catch (Exception e) {
       throw new ExecutionTerminator();
     } 
