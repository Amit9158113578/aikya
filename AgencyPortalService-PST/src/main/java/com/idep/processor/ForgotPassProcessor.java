...
public class ForgotPassProcessor implements Processor{

	ObjectMapper objectMapper = new ObjectMapper();
	Logger log = Logger.getLogger(ForgotPassProcessor.class.getName());
	    	...
	    	if(reqNode.has("action")&& reqNode.get("action").asText().equals("FORGOTPASS"))
	    	{
		    	if(reqNode.has("mobile")&&reqNode.has("email")&&reqNode.has("newPass"))
		    	{
			    	try{
				        ...
				        if(AgencyConstants.TRIGGER_EMAIL_SEND_MSG !=status){
