...
public class SyncGatewayServices {
  ObjectMapper objectMapper = new ObjectMapper();
  
  Logger log = Logger.getLogger(SyncGatewayServices.class.getName());
  
  public JsonNode getTransDocumentBySync(String docid) {
    JsonNode documentNode = null;
    DefaultHttpClient httpclient = new DefaultHttpClient();
    try {
      HttpGet httpGet = null;
      httpGet = new HttpGet(String.valueOf(SyncGatewayConfigLoader.syncTransPublicURL) + docid);
  ...
  public String createTransDocumentBySync(String docid, JsonObject jsonobj) {
    DefaultHttpClient httpclient = new DefaultHttpClient();
    try {
      jsonobj.put("_id", docid);
      HttpPost httppost = null;
      httppost = new HttpPost(SyncGatewayConfigLoader.syncTransPublicURL);
  ...
  public String replaceTransDocumentBySync(String docid, JsonObject jsonobj) {
    DefaultHttpClient httpclient = new DefaultHttpClient();
    try {
      JsonNode documentNode = getTransDocumentBySync(docid);
      if (documentNode != null) {
        jsonobj.put("_id", docid);
        HttpPut httpput = null;
        httpput = new HttpPut(String.valueOf(SyncGatewayConfigLoader.syncTransPublicURL) + docid + "?new_edits=true&rev=" + documentNode.get("_rev").asText());
        StringEntity input = new StringEntity(jsonobj.toString(), ContentType.APPLICATION_JSON);
  ...
  public String deleteTransDocumentBySync(String docid) {
    DefaultHttpClient httpclient = new DefaultHttpClient();
    try {
      JsonNode documentNode = getTransDocumentBySync(docid);
      if (documentNode != null) {
        HttpDelete httpDelete = null;
        httpDelete = new HttpDelete(String.valueOf(SyncGatewayConfigLoader.syncTransPublicURL) + docid + "?new_edits=true&rev=" + documentNode.get("_rev").asText());
  ...
  public JsonNode executeTransSyncGatewayView(String viewName, String key) {
